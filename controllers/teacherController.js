...
function generateExamToken() {
  ...
  async function refreshExamToken(req, res) {
    try {
      ...
      res.status(200).json({ message: 'Exam token refreshed', examToken: newExamToken });
    } catch (error) {
  ...
  function calculateCosineSimilarity(text1, text2) {
    ...
    const similarity = natural.Vector.dot(vec1, vec2) / (natural.Vector.magnitude(vec1) * natural.Vector.magnitude(vec2));
    return similarity;
  }
  
  // ...
  
  module.exports = { createExam, exportStudentAnswers,refreshExamToken , evaluateExam };